...
N = int(os.environ.get("N_ITEMS", "400")); K_FLOW = int(os.environ.get("K_FLOW", "4"))
...
OUT = os.environ.get("OUT", "/mnt/pfs/yuchen/groot/fit_dump_gr1_v2.npz")
...
cfg0 = make_config(lt0, float(df0), int(mvt0), ckpt0, f"/mnt/pfs/yuchen/groot/fitdump2_{name0}")
...
batches, idents = [], []
for k in range(N):
    b = next(it); inner = b["inputs"] if "inputs" in b else b
    batches.append(to_dev(inner, "cpu")); idents.append(LAST["ident"] or ("?", -1, -1))
    if k % 100 == 0: print(f"cached {k}/{N} ident={idents[-1]}", flush=True)
...
for j, (name, ckpt, lt, df, mvt) in enumerate(specs):
    if j == 0: model = pipe0.return_model()
        ...
        cfg = make_config(lt, float(df), int(mvt), ckpt, f"/mnt/pfs/yuchen/groot/fitdump2_{name}")
...
print("FITDUMP2_DONE", flush=True)
