"""Kronecker-factored covariance for the GR00T HT loss.
...
Sigma_A is renormalised to |Sigma_A| = 1, which fixes the (c*Sigma_T) (x) (Sigma_A/c) degeneracy and
puts the whole scale in Sigma_T. The diagonal biases are therefore chosen FLOOR-AWARE so that at a
zero head output the effective scale equals the isotropic branch's sigma exactly
(sigma_iso = softplus(ht_sbias) + 1e-3), i.e. Sigma = sigma_iso^2 I and the loss coincides with the
isotropic multivariate-t loss this arm is compared against.

Parameters come from the existing sigma head pooled over each sample's VALID timesteps, so every
sample gets its own Sigma_T, Sigma_A and no new parameters are introduced.
"""
import shutil

P = "/mnt/pfs/yuchen/groot/Isaac-GR00T/gr00t/model/gr00t_n1d7/gr00t_n1d7.py"
shutil.copy(P + ".bak_prekron", P)          # always start from the unpatched file
src = open(P).read()
...
print("patched; kron branch present:", "[ht-kron]" in open(P).read())
