...
fig, axes = plt.subplots(1, 4, figsize=(20.5, 6.2))

ARMS = [("L2", "l2"), ("HG", "hg"), ("HT", "ht"), ("flow (tuned)", "flow8")]
for ax, (label, arm) in zip(axes, ARMS):
    if arm == "flow8":
        os.environ["WIDTH"] = "512"
        os.environ["STEPS"] = "150000"
        os.environ["EMA"] = "0.999"
        T.WIDTH, T.STEPS, T.EMA_R = 512, 150000, 0.999
    else:
...
fig.suptitle("z-turn failure modes by arm (30 traced rollouts each; "
             "red = scrape with final contact marked, grey = timeout)",
             fontsize=12.5)
fig.tight_layout(rect=[0, 0, 1, 0.94])
fig.savefig("arms_fail_fig.png", dpi=160)
print("wrote arms_fail_fig.png", flush=True)
