...
NOISE_TYPE = os.environ.get("NOISE_TYPE", "none")  # none|dir|speed|both|zturn|hold, composable via "+"
TYPES = (set() if NOISE_TYPE == "none"
         else {"dir", "speed"} if NOISE_TYPE == "both"
         else set(NOISE_TYPE.replace(",", "+").split("+")))
HOLD_MAX = int(os.environ.get("HOLD_MAX", "24"))  # hold type: per-episode pause draw
...
def noise_band(y):
    ...
    return (("dir" in TYPES and in_dir_band(y))
            or ("speed" in TYPES and in_speed_band(y))
            or ("zturn" in TYPES and in_z_band(y))
            or ("hold" in TYPES and abs(y - Y_HOLD) <= 0.03))
...
def gen_episode(rng):
    ...
    if "zturn" in TYPES:
        ...
        t1, t2 = T1_FIX, T2_FIX
    held = 0
    h_ep = rng.randint(0, HOLD_MAX + 1) if "hold" in TYPES else 0
    for _ in range(300):
        a = expert_action(p, t1, t2)
        if HOLD > 0 and held < HOLD and p[1] <= Y_HOLD:
            a = np.array([0.0, 0.0])
            held += 1
        if "speed" in TYPES and in_speed_band(p[1]):
            if rng.rand() < PAUSE_P:
                eta = 0.0
            else:
        ...
        if "dir" in TYPES and in_dir_band(p[1]):
            a[0] = float(np.clip(a[0] + S_HI * rng.standard_t(2), -CLIP, CLIP))
        if "hold" in TYPES and held < h_ep and p[1] <= Y_HOLD:
            a = np.array([0.0, 0.0])
            held += 1
...
class FlowNet(nn.Module):
    ...
    def forward(self, xn):
        if os.environ.get("FLOW_RAND", "0") == "1":
            # proper sampling: integrate from a source draw (matches training
            # interpolant y_t = (1-t) eps + t y and the robot samplers)
            y = torch.randn(len(xn), ACT_D * H, device=xn.device)
        else:
...
LR_COS = os.environ.get("LR_COS", "0") == "1"
EMA_R = float(os.environ.get("EMA", "0"))  # 0 = off; e.g. 0.999
...
def train(arm, seed, Xn, Yn):
    ...
    sched = (torch.optim.lr_scheduler.CosineAnnealingLR(
        opt, STEPS, eta_min=LR * 0.01) if LR_COS else None)
    ema = _copy.deepcopy(net) if EMA_R > 0 else None
        ...
        if ema is not None:
            with torch.no_grad():
                for pe, p in zip(ema.parameters(), net.parameters()):
                    pe.mul_(EMA_R).add_((1 - EMA_R) * p)
    return ema if ema is not None else net
