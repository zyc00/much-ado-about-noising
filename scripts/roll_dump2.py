...
DS = os.environ.get("DDS", "data/tool_hang_full2ins_mp_200.hdf5")
...
for sd in range(21000, 21000 + n):
    ...
    while steps < 700 and not asm:
        for a in chunk_of(hist):
            ...
            if steps >= 700: break
    succ += int(asm)
    np.savez(f"{out}/ep_{sd}.npz", obs=np.array(hist, dtype=np.float32), eef=np.array([h[44:47] for h in hist]), asm=int(asm), seed=sd, steps=steps)
