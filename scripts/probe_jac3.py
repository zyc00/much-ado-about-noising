...
Q = np.load(os.environ.get("QUERIES", "scripts/jac_queries.npz"))
PSD = os.environ.get("PERSTATE")
for qname in ["canon", "dep"]:
    ...
    for w in Q[qname]:
        ...
        for b, ncol, dl in spec:
            ...
            if PSD and b in ("eef", "frame", "grip"):
                eig = np.linalg.eigvalsh((Jp @ Jp.T) * 0) if False else (np.linalg.eigvalsh((Jp + Jp.T) / 2).max() if Jp.shape[0] == Jp.shape[1] else float("nan"))
                print(f"PS {os.environ.get('TAG','')} {qname} {b} {len(stats[b]['fro'])-1} {np.linalg.norm(Jp):.3f} {eig:.3f}")
