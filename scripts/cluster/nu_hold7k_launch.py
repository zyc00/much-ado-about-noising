...
VALUES = (1024, 512, 256, 128, 64, 32, 14)
...
def nu_at_step(completed_steps):
    ...
    return float(VALUES[min(max(0, completed_steps - 6000) // 1000, len(VALUES) - 1)])
    ...
    class StaircaseNu(TrainerCallback):
        ...
        def update(self, args, state, model):
            ...
            for cfg in (core.config, core.action_head.config):
                ...
                cfg.ht_df = nu
                cfg.ht_step_schedule = {'values': list(VALUES), 'interval': 1000, 'hold_steps': 7000}
            if nu != self.last_nu:
