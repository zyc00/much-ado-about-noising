...
code=/mnt/pfs/yuchen/nu224_exp_20260909/code
run_dir="$root/ft_fr_nu224_exp_20260909"
staged=/dev/shm/fr_nu224_exp
cd "$root/Isaac-GR00T"
...
mkdir "$run_dir"
mkdir "$run_dir/audit"
cp "$code/nu224_exp_launch.py" "$code/nu224_exp_train.sh" \
   "$code/test_nu224_exp.py" "$code/nu_recipe_fork_eval.py" \
...
python "$run_dir/audit/test_nu224_exp.py" 2>&1 | tee "$run_dir/audit/unit_tests.log"
mkdir "$staged"
...
torchrun --nproc_per_node=8 --master_port=29500 "$run_dir/audit/nu224_exp_launch.py" \
...
test -f "$run_dir/initialization_verified.json"
# Match the original nu=224 result: SEED=1234, six tasks, 100 rollouts/task,
# five vector environments, NAS=1, and 300 maximum environment steps.
