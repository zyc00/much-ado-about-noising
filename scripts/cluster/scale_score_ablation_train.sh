#!/usr/bin/env bash
set -euo pipefail
arm="${SCALE_SCORE_ARM:?must specify ablation arm}"
case "$arm" in se_exp|se_softplus|ht_exp_gaussaux1) ;; *) exit 2 ;; esac
root=/mnt/pfs/yuchen/groot
code=/mnt/pfs/yuchen/scale_score_ablation_20260908/code
run_dir="$root/ft_fr_nu14_${arm}_20260908"
staged="/dev/shm/fr_scale_score_${arm}"
cd "$root/Isaac-GR00T"
...
mkdir "$run_dir"
mkdir "$run_dir/audit"
cp "$code/scale_score_ablation_launch.py" "$code/scale_score_ablation_train.sh" \
   "$code/test_scale_score_ablation.py" "$code/nu14_gaussian_aux_launch.py" \
   "$code/nu_recipe_fork_eval.py" "$code/nu_fixed_scenes_rollout.py" \
   "$code/nu_hold7k_server.py" "$code/README.md" "$run_dir/audit/"
...
cmp "$run_dir/audit/model_source.py" "$root/ft_fr_nu14_gaussaux1_fresh_20260908/audit/model_source.py"
cmp "$run_dir/audit/trainer_source.py" "$root/ft_fr_nu14_gaussaux1_fresh_20260908/audit/trainer_source.py"
python "$run_dir/audit/test_scale_score_ablation.py" 2>&1 | tee "$run_dir/audit/unit_tests.log"
mkdir "$staged"
...
torchrun --nproc_per_node=8 --master_port=29500 "$run_dir/audit/scale_score_ablation_launch.py" \
...
test -f "$run_dir/initial_batch_verified.json"
