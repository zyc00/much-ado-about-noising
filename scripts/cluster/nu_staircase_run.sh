...
scripts="$root/nu_staircase_20260907"
case "$dataset" in
  widowx)
...
run_dir="$root/ft_${short}_nustair_20260907"
...
cp "$scripts/nu_staircase_launch.py" "$scripts/nu_staircase_run.sh" "$scripts/nu_staircase_eval.py" "$run_dir/audit/"
...
torchrun --nproc_per_node=8 --master_port=29500 "$scripts/nu_staircase_launch.py" \
  ...
  --no-use-wandb --loss-type=hetero_t --ht-sbias="$sbias" --ht-df=1792.0 \
  --ht-mvt --state-dropout-prob "$dropout" \
  2>&1 | tee "$run_dir/train.log"
python "$scripts/nu_staircase_eval.py" --dataset "$dataset" --run-dir "$run_dir" \
