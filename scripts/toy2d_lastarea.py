"""Last-area cell: detour pattern everywhere EXCEPT the final stretch.
...
AIM = float(os.environ.get("LA_AIM", "2.0"))       # detour amplitude (mm)
CLEAN_X = float(os.environ.get("LA_CLEAN", "130.0"))  # no detours beyond this x
HORIZON = 8
MAX_CHUNKS = 70
...
def gen_episodes(n_episodes, seed):
    ...
    for _ in range(n_episodes):
        ...
        while x < X_GOAL_MM:
            ...
            # zero-mean but SKEWED detour aim: 80% at -AIM, 20% at +4*AIM
            aim = (4.0 * AIM) if rng.random() < 0.2 else (-AIM)
            ch = np.zeros(2 * HORIZON, dtype=np.float32)
            xx, yy = x, y
            for j in range(HORIZON):
                yt = 0.0 if xx >= CLEAN_X else aim   # last area: no detour
                dy = -K_SERVO * (yy - yt)            # EXECUTED (no injection)
...
def main():
    ...
    Path(os.environ.get("NU_OUT", "analysis/toy2d_lastarea.json")).write_text(json.dumps(out, indent=1))
    print("WROTE", os.environ.get("NU_OUT", "analysis/toy2d_lastarea.json"), flush=True)
