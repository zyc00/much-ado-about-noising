...
SNAPS = [int(x) for x in os.environ.get("RR_SNAPS", "").split(",") if x]
...
for e in range(len(ends)):
    ...
    A_ = rb["action"][s0:e0]
    for i in range(1, e0 - s0 - H, 5):
        Yl.append(ds0.normalizer["action"].normalize(A_[i:i + H]))
Ytr = torch.tensor(np.stack(Yl)[sel], device=dev, dtype=torch.float32)

for name, loss, ck in ARMS:
    cfg, ds, ag = load(loss)
    for snap in (SNAPS or [0]):
        ckp = ck if not SNAPS else ck.replace("snap_300000", f"snap_{snap}")
        if not os.path.exists(ckp):
            continue
        ag.load(ckp, load_optimizer=False)
        ag.eval()
        _run(name, loss, ag, snap)


def _run(name, loss, ag, snap):
    with torch.no_grad():
    ...
    with torch.no_grad():
        ...
        pr_a, _ = ag.flow_map_ema.net(a0, t0, t0, ag.encoder_ema(
            {"state": X.reshape(-1, 2, 53)}, None))
        tr_err = float((pr_a - Ytr).abs().mean())
    ...
    print(f"RREP {name} snap {snap} emb_PR {pr(ev):.2f} emb_k90 {k90} "
          f"train_err {tr_err:.5f} W_srank {[round(s,1) for s in sr]} "
