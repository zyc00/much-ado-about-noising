...
state, action = make_dataset("midlock", 3000, 10, 8, 20260805)
arms = {}
...
SPECS = [("Regression", reg, "regression", "tab:blue"),
         ("HT $\\nu$=2 (standard)", ht2, "ht", "tab:green"),
         ("MIP step 1", mip, "mip_step1", "#8c6bb1"),
...
ax.set_title("Midlock witness rollouts (seed 0, 11 starts per method): only HT "
             "$\\nu$=2 docks",
             fontsize=11.5, loc="left")
axz.set_xlim(125, 168)
axz.set_ylim(-4.6, 1.1)
axz.set_xlabel("x (mm)")
axz.set_title("Dock zoom (midlock): only standard HT $\\nu$=2 centers; "
              "L2 and MIP step 1\nsettle on the bottom branch "
              "($-$3.6 mm), MIP full between ($-$1.9 mm)",
              fontsize=10.5, loc="left")
fig.tight_layout()
fig.savefig("analysis/paper/toy_midlock_rollouts.png", dpi=145,
            bbox_inches="tight")
print("saved")
