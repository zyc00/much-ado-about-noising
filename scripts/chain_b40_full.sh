...
PY=./.venv/bin/python
# b40 control
rm -rf logs/b40_regression_2000
MUJOCO_GL=egl $PY -u examples/train_robomimic.py task=tool_hang_ph_state_delta_legacy \
  +task.dataset_path=$(pwd)/data/tool_hang_b40_2000.hdf5 task.num_envs=1 network=chiunet \
  optimization.loss_type=regression optimization.auto_resume=false log.wandb_mode=disabled \
  log.eval_freq=100000000 log.save_freq=100000 log.exp_name=b40_regression_2000 \
  log.log_dir=logs/b40_regression_2000 > /tmp/b40_regression_2000.log 2>&1
echo "=== b40_regression_2000 TRAINED ==="
MUJOCO_GL=egl $PY scripts/eval_insertion.py --ckpt logs/b40_regression_2000/models/model_latest.pt \
  --dataset data/tool_hang_b40_2000.hdf5 --eval_states data/b40_eval_states.hdf5 \
  --loss regression --n 100 --max_chunks 30 2>&1 | grep -aE "INSERTION_SR" | sed "s/^/Bsweep B=40 N=2000 /"
# full_mip
