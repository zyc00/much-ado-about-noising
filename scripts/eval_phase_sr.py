"""Standalone SR for the oracle-phase policy (56-dim obs): phase one-hot driven online
by the label convention -- [1,0,0] until closure+16, [0,1,0] until closure+70, then [0,0,1].
Env: CKPT/TAG."""
import os, sys
...
DS = "data/tool_hang_full2ins_2000_phase.hdf5"
...
def load_agent(loss, ck, ds_path=None):
    ...
    return cfg, ds, ag
cfgA, dsA, agA = load_agent("regression", os.environ["CKPT"])
cfgB, dsB, agB = cfgA, dsA, agA
...
PH = [np.array([1.,0.,0.]), np.array([0.,1.,0.]), np.array([0.,0.,1.])]
def ov(o, ph): return np.concatenate([np.concatenate([o[KM.get(k, k)] for k in OK]), ph]).astype(np.float32)
...
for sd in range(21000, 21048):
    ...
    o = env._get_observations(force_update=True); hist = [ov(o, PH[0]), ov(o, PH[0])]
    ...
    while steps < 700 and not asm:
        ...
        for a in ch:
            ...
            phi = 0 if (closed_at is None or steps < closed_at + 16) else (1 if steps < closed_at + 70 else 2)
            hist.append(ov(o, PH[phi]))
...
print(f"PHASE-SR {os.environ.get('TAG','')}: SR={succ}/{N}")
print("SPLICE-DONE")
