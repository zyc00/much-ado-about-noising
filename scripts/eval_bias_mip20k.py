"""Capture MIP-20k bias on the SAME test states / clean-support metric, then print
the full 5-way bias(d) table."""
import os
os.environ["MUJOCO_GL"] = "egl"
import argparse, numpy as np, torch, h5py, sys
...
ap.add_argument("--ckpt", default="logs/full_mip_20000/models/model_latest.pt")
...
args = ap.parse_args()
cfg, ds, ag = load(args.ckpt, args.ds, "mip")
...
for ov, acts in test:
    ...
    for t in range(1, T):
        ...
        eP.append(predict(win)[:, :6] - gt); dists.append(dist(ov[t]))
dists = np.array(dists); eP = np.array(eP)
print(f"captured {len(dists)} states; err shape {eP.shape}")
np.savez("analysis/recovery/err_mip20k.npz", dist=dists, eP20=eP)
ref = np.load("analysis/recovery/err_vectors.npz"); rd = ref["dist"]; rM = ref["eMSE"][:,0,:]; rP = ref["eMIP"][:,0,:]
c20 = np.load("analysis/recovery/err_clean20k.npz"); cd = c20["dist"]; e20 = c20["e20"][:,0,:]
dk = np.load("analysis/recovery/err_dart6k.npz"); dD = dk["dist"]; eD = dk["eDART"][:,0,:]
p20 = eP[:,0,:]
print(f"\n{'bin':10} {'MSE-2k':>8} {'MSE-20k':>8} {'MIP-2k':>8} {'MIP-20k':>8} {'DART-6k':>8}")
for lo, hi in [(0,1),(1,2),(2,3),(3,4),(4,6),(6,1e9)]:
    def b(d,e):
        m=(d>=lo)&(d<hi); return np.linalg.norm(e[m].mean(0)) if m.sum()>=10 else float("nan")
    print(f"[{lo},{hi})    {b(rd,rM):>8.3f} {b(cd,e20):>8.3f} {b(rd,rP):>8.3f} {b(dists,p20):>8.3f} {b(dD,eD):>8.3f}")
print("\nsaved analysis/recovery/err_mip20k.npz")
