...
PAIRS = [("hMSE_s5", "hMIP_s5", "chiunet",
           "tool_hang_ph_state_regression_chiunet_256_seed5_success52.pt",
           "tool_hang_ph_state_mip_chiunet_256_seed5_success82.pt"),
          ("hMSE_s5001", "hMIP_s5001", "chiunet",
           "tool_hang_ph_state_regression_chiunet_256_seed5001_success60.pt",
           "tool_hang_ph_state_mip_chiunet_256_seed5001_success80.pt"),
          ("hMSEmlp_s5001", "hMIPmlp_s5001", "mlp",
           "tool_hang_ph_state_regression_mlp_512_seed5001_success90.pt",
           "tool_hang_ph_state_mip_mlp_512_seed5001_success85.pt")]
PAIRS = [p for p in PAIRS if os.path.exists(f"analysis/traj_vis/human_{p[0]}.npz") and os.path.exists(f"analysis/traj_vis/human_{p[1]}.npz")]
print("pairs available:", [p[0] for p in PAIRS], flush=True)
Z = {}
for p in PAIRS:
    Z[p[0]] = np.load(f"analysis/traj_vis/human_{p[0]}.npz")
    Z[p[1]] = np.load(f"analysis/traj_vis/human_{p[1]}.npz")
def eps_of(z):
    out = []
    i = 0
...
print("=== A. OUTCOME MATRICES (matched seeds)")
for pm, pp, net, _, _ in PAIRS:
    wins = {"mip": 0, "mse": 0, "both": 0, "neither": 0}
    for j in range(min(len(EPS[pm]), len(EPS[pp]))):
        s1 = int(EPS[pm][j][3][0]); s2 = int(EPS[pp][j][3][0])
        k = "both" if s1 and s2 else ("mip" if s2 else ("mse" if s1 else "neither"))
        wins[k] += 1
    print(f"OUTCOME {pm.replace('hMSE','')}: MIP-only={wins['mip']} MSE-only={wins['mse']} both={wins['both']} neither={wins['neither']}", flush=True)

# ---- B. drift field + C. mode taxonomy
print("=== B/C. DRIFT FIELD + MODES")
BINS = [(0, 1), (1, 2), (2, 3), (3, 5), (5, 1e9)]
for mname in list(EPS.keys()):
...
POL = {}
NETOF = {}
for pm, pp, net, ckm, ckp in PAIRS:
    for name, loss, ck in [(pm, "regression", f"{OLD}/{ckm}"), (pp, "mip", f"{OLD}/{ckp}")]:
        c_, d_, ag = load(loss, net)
...
def pol_act(name, X):
    enc, fm = POL[name]
    CH = 10 if NETOF[name] == "mlp" else 16
    preds = []
...
# MSE stall sequences per pair
for pm, pp, net, _, _ in PAIRS:
  segs = []
  for o, a, p, m in EPS[pm]:
  ...
  segs = segs[:60]
  print(f"stall segments {pm}: {len(segs)}", flush=True)
  agree, magr, tv_m, tv_p, gtal_m, gtal_p = [], [], [], [], [], []
  for W in segs:
    am = pol_act(pm, W); ap = pol_act(pp, W)
  ...
  if segs:
    print(f"REPLAY {pm}: cos(a_MSE,a_MIP) p50={np.median(agree):+.2f} | |a_MIP|/|a_MSE| p50={np.median(magr):.2f} | relTV MSE={np.median(tv_m):.2f} MIP={np.median(tv_p):.2f} | GT-align MSE={np.median(gtal_m):+.2f} MIP={np.median(gtal_p):+.2f}", flush=True)
