...
Usage:
  MUJOCO_GL=egl python scripts/collect_insertion_only.py \
     ...
     --output data/tool_hang_aligninsert_20000.hdf5
"""
import os
...
def main():
    ...
    ap.add_argument("--output", default="data/tool_hang_aligninsert_20000.hdf5")
    ...
    demos = []
    attempts = 0
    pbar = tqdm(total=args.n_demos, desc="align+insert")
    ki = 0
    while len(demos) < args.n_demos and ki < len(src_keys):
        d = sf["data/" + src_keys[ki]]; ki += 1
        af0 = align_done_frame(d)
        if af0 is None:
            continue
        # back off a random 0..backoff_max steps before align-done -> start
        # includes part of the align sub-skill (same script then takes over).
        # guard >= grasp (c1+2). align span (c1->align_done) is ~80 steps.
        a=d["actions"][:]; g=a[:,6]
        c1=next((t for t in range(1,len(g)) if g[t-1]<0 and g[t]>=0), 0)
        backoff=int(np.random.RandomState(int(d.attrs.get("seed",0))).randint(0,args.backoff_max+1))
        af=max(c1+2, af0-backoff)
    ...
    pbar.close()
    sf.close()
    print(f"Collected {len(demos)} aligninsert demos from {attempts} attempts ({100*len(demos)/max(attempts,1):.0f}%)")
