"""Cancelling-cycle cell: the mean-family witness, fully BEHAVIOURAL.
...
def gen_episodes(n_episodes, seed):
    ...
    for _ in range(n_episodes):
        ...
        phase = int(rng.integers(0, 5))   # hidden phase: 1 chunk of history cannot identify it
        prev = np.zeros(2 * HORIZON, dtype=np.float32)
        while x < X_GOAL_MM:
            st = np.array([x, y], dtype=np.float32)
            # deterministic cancelling cycle: [-b,-b,-b,-b,+4b] repeating.
            # Executed (not injected); it cancels exactly, so demos dock.
            d = (4.0 * B) if (phase % 5 == 4) else (-B)
            phase += 1
            ch = np.zeros(2 * HORIZON, dtype=np.float32)
            xx, yy = x, y
            for j in range(HORIZON):
                dy = -K_SERVO * yy + d * ACTION_MM   # EXECUTED offset
...
def main():
    ...
    Path(os.environ.get("NU_OUT", "analysis/toy2d_cycle.json")).write_text(json.dumps(out, indent=1))
    print("WROTE", os.environ.get("NU_OUT", "analysis/toy2d_cycle.json"), flush=True)
